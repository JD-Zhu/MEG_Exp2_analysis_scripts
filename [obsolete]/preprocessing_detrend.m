...
function [all_blocks, trialinfo_b] = preprocessing_detrend(SubjectFolder, lay, chanlocs)
    ...
    output_path = [SubjectFolder 'output\\'];
    mkdir(output_path);
    ...
    for i = 1:length(files)
        ...
        %output_file = [output_path 'hpfilt0.5+-0.1_B' num2str(i) '.mat'];
...
%TODO - trying detrend on continuous data:    
        output_file = [output_path 'after_locdetrend_on_cont_data.mat'];
        if (exist(output_file, 'file') ~= 2)    
            ...
            output_file = [output_path 'data4eeglab_B' num2str(i) '.mat'];
            ...
            save([output_path 'after_locdetrend_on_cont_data.mat'], 'alldata');
        else
            load(output_file);
        end
...
%TODO - trying a few detrend options:
        % <1> FT_detrend
        %{
